...
if ( !class_exists( 'avia_sc_notification_1' ) ) 
{
	class avia_sc_notification_1 extends aviaShortcodeTemplate
	{
			/**
			...
			{
				$this->config['name']		= __('PPC Color Box', 'avia_framework' );
//				$this->config['tab']		= __('Content Elements', 'avia_framework' );
				...
				$this->config['shortcode'] 	= 'ppc_notification';
			...
			function shortcode_handler($atts, $content = "", $shortcodename = "", $meta = "")
			{
				...
				$output .= "<div id='ppc-note' {$style} class='avia_message_box ".$this->class_by_arguments('color, size, icon_select, border' , $atts, true).$meta['el_class']."'>";
				
				
				if($atts['title']) 
				{
					$output .= "<span class='ppc_message_box_title' >".$atts['title']."</span>";
				}
...
//				$output .=	"<div {$style} class='ppc-border'><div class='special-heading-inner-border' style='border-color:#ffffff'></div></div>";
//				$output .= "</div>";
